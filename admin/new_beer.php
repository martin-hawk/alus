...
&& $imageFileType != "gif" ) {
		...
		if ($uploadOk != 0) {
			$sql = "INSERT INTO beers (`brewer_id`, `image`, `name`, `active`, `show_name`, `description`, `show_description`) VALUES ('$brewer_id', '$image', '$name', '$active', '$show_name', '$description', '$show_description')";
	...
	$sql = "INSERT INTO beers (`brewer_id`, `name`, `active`, `show_name`, `description`, `show_description`) VALUES ('$brewer_id', '$name', '$active', '$show_name', '$description', '$show_description')";
	$conn->query($sql);
	$errors['success'] = 'Duomenys išsaugoti sėkmingai!';
	$conn->close();
	}
...
<section class="section-style w3-text-orange" style="width:66%;">
  <h3 style="border-bottom:solid thin #FF9800;">Kurti produktą</h3>
    ...
    <table class="w3-table" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <?php
			...
			$sql = "SELECT id, name FROM brewers ORDER BY name ASC";
        ...
        <td rowspan="3" align="center" valign="middle"><input type="submit" class="w3-btn w3-light-grey" name="save" value="Išsaugoti"></td>
      </tr>
      <tr>
        <td style="text-align:left;"><label for="image">Paveikslėlis:</label></td>
        <td style="text-align:left;"><input type="file" name="image" id="image" size="32"></td>
      </tr>
      <tr>
        <td style="text-align:left;"><label for="name">Pavadinimas:</label></td>
        <td style="text-align:left;"><input type="text" name="name" size="32"></td>
      </tr>
      <tr>
        <td style="text-align:left;"><label for="description">Aprašymas:</label></td>
        <td style="text-align:left;"><textarea name="description"></textarea></td>
        <td style="text-align:left;"><input type="checkbox" value="1" name="active" checked>
          <label for="show_name">Rodyti gaminį</label>
          <br>
          <input type="checkbox" value="1" name="show_name" checked>
          <label for="show_name">Rodyti pavadinimą</label>
          <br>
          <input type="checkbox" value="1" name="show_description">
