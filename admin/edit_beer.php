...
if (isset($_POST['update']) == 'Atnaujinti') {
	...
	$brewer_id = $_POST['brewer_id'];
	$name = $conn->real_escape_string($_POST['name']);
	if (!isset($_POST['active'])) $active = '';
	else $active = $_POST['active'];
	...
	$sql = "UPDATE beers SET brewer_id = '$brewer_id', name = '$name', active = '$active', show_name = '$show_name', description = '$description', show_description = '$show_description' WHERE id = " . $_GET['id'];
	$conn->query($sql);
...
&& $imageFileType != "gif" ) {
		...
		if ($uploadOk != 0) { 
			$sql = "UPDATE beers SET brewer_id = '$brewer_id', active = '$active', image = '$image', name = '$name', show_name = '$show_name', description = '$description', show_description = '$show_description' WHERE id = " . $_GET['id'];
...
if (isset($_POST['delete']) == 'Ištrinti') {
	...
	$sql = "DELETE FROM beers WHERE id = '" . $_GET['id'] . "'";
	$conn->query($sql);
	
	$errors['success'] = 'Produktas buvo sėkmingai ištrintas!';
...
<section class="section-style w3-text-orange" style="width:66%;">
  <h3 style="border-bottom:solid thin #FF9800;">Taisyti produktą</h3>
  ...
  <form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
	...
	$sql = "SELECT * FROM beers WHERE id = " . $_GET['id'];
	$result = $conn->query($sql);
	
	$helper_sql = "SELECT id, name FROM brewers ORDER BY name ASC";
    ...
    <table class="w3-table" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td style="text-align:left;"><label for="brewer_id">Aludaris:</label></td>
        <td style="text-align:left;"><select name="brewer_id">
        <?php
			if ($helper_result->num_rows > 0) {
				while($helper_row = $helper_result->fetch_assoc()) {
					echo ($helper_row['id']);
					if ($helper_row['id'] == $row['brewer_id']) echo '<option value="' . $helper_row['id'] . '" selected>' . $helper_row['name'] . '</option>';
					else echo '<option value="' . $helper_row['id'] . '">' . $helper_row['name'] . '</option>';
				}
			}
		?>
        </select></td>
        <td rowspan="3" align="center" valign="middle"><input type="submit" class="w3-btn w3-light-grey" name="update" value="Atnaujinti"><p></p><input type="submit" class="w3-btn w3-light-grey w3-hover-red" name="delete" value="Ištrinti" ></td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td style="text-align:left;"><input type="checkbox" value="1" name="active" <?php if ($row['active'] == true) echo 'checked'; ?>>
          <label for="show_name">Rodyti gaminį</label>
          <br>
          <input type="checkbox" value="1" name="show_name" <?php if ($row['show_name'] == true) echo 'checked'; ?>>
